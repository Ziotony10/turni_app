...
```
    ...
    # ── NUOVA: tabella visite pagina login ────────────────────────────────
    ...
    if not USE_PG:
        ...
        # PostgreSQL: l'ALTER TABLE va in una transazione separata
        # per non annullare le CREATE TABLE eseguite sopra
        conn.commit()  # salva tutte le CREATE TABLE prima
        try:
        ...
        try:
            ...
            conn.rollback()
        # Crea login_page_visits su PG se non esiste (IF NOT EXISTS è sicuro)
        try:
            ex(conn, "SELECT 1 FROM login_page_visits LIMIT 1")
        except Exception:
            conn.rollback()
            ex(conn, """CREATE TABLE IF NOT EXISTS login_page_visits (
